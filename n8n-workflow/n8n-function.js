...
if(!uProc && !hunterData) {
    return [{
        json: {
            spam: "maybe",
            confidence: "low",
            reason: "Could not get any verification record for the provided email at the moment.",
            result: "vetting"
        }
    }];
} else {

    if (!uProc.result || ( hunterData && (hunterData.disposable || hunterData.webmail))) { // Hunter uses boolean flags for these
        spamStatus = "yes";
    }
...
// Hunter provides a confidence score from 0-100
    ...
    if (!uProc.result && webhookData.emailDomain !== uProc.message.email) {
        spamStatus = "yes";
        confidenceLevel = "high";
    }
